exports.handler = async function(event, context) {
  ...
          statusCode: 500, 
          headers, 
          body: JSON.stringify({ error: 'System-Fehler: Der API-Key (GEMINI_API_KEY) wurde auf Netlify nicht gefunden.' }) 
        };
      }
  ...
      // Chat-History in das Google Gemini Format übersetzen (user -> user, assistant -> model)
  ...
      if (!response.ok) {
        console.error("Google API Chat-Fehler:", data);
        return { 
          statusCode: response.status, 
          headers, 
          body: JSON.stringify({ 
            error: data.error?.message || 'Fehler bei der Verbindung mit dem Gemini Chatbot.' 
          }) 
        };
      }
